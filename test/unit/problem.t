...
my $pheno_order = [ 'problem', 'input', 'data', 'subroutine', 'pk', 'error', 'theta', 'omega', 'sigma', 'estimation', 'covariance', 'table' ];
is_deeply ($problem->own_print_order, $pheno_order, "own_print_order");

# what to add, remove, starting, result
my @test_orders = (
    [
        [], [],
...
foreach my $test (@test_orders) {
    my $model = model->new(filename => "$modeldir/pheno.mod");
    my $problem = $model->problems->[0];
    foreach my $record (@{$test->[1]}) {
        my $attribute = $record . 's';
        $problem->$attribute([]);
    }
    foreach my $record (@{$test->[0]}) {
        $problem->add_records(type => $record, record_strings => [ ' DUMMY' ]);
    }

    my $po = $problem->create_print_order(own_print_order => $test->[2]);
    is_deeply($po, $test->[3], "create_print_order removing " . join(", ", @{$test->[1]}) .  " and adding " . join(", ", @{$test->[0]}));
}
