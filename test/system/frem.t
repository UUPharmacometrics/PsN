...
#	get_command('frem') . " -time_var=WT -occ=VISI -param=PHI,LAG -invar=SEX,DGRP -no-check $model_dir/mox_no_bov.mod ",
	get_command('frem') . " -covar=WT,DGRP,SEX -skip_omegas=2  -log=WT -categorical=DGRP -check $model_dir/mox_frem.mod -estimate_reg -no-run_sir",
	get_command('frem') . " -covar=WT,DGRP -categorical=DGRP -no-check $model_dir/mox_frem.mod -estimate_reg -no-run_sir ",
	get_command('frem') . " -covar=DIG,WT -no-check $model_dir/mox_frem.mod -estimate_reg -no-run_sir",
	get_command('frem') . " -covar=SEX,DGRP -skip_om=2 -categorical=SEX,DGRP -cholesk -estimate_reg -no-run_sir -no-check $model_dir/mox1.mod ",
	get_command('frem') . " -covar=AGE,SEX -categorical=SEX -no-check $model_dir/mox1.mod -estimate_reg -no-run_sir",
