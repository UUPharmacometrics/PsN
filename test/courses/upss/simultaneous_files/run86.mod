...
$DES
 ...
 CDE = A(3)/S3     ; Effect compartment concentration
 ...
 DADT(4) = BHAZ*EXP(-CDE*THETA(8))     ; Integrate hazard
...
IF(FLG.EQ.3)    IPRED = SUR*BHAZ*EXP(-CE*THETA(8))  ; Survival * hazard (S*h), conversion
...
$THETA  .001             ; 8 CE effect on Hazard
