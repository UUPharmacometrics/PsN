...
$DES
 ...
 BHAZ = THETA(7)
 IF(HR.GT.100.AND.HR.LE.120) BHAZ = THETA(8) ; Hazard depends on HR
 IF(HR.GT.120) BHAZ = THETA(9)    
...
;------------TIME TO EVENT--------------     
  ...
  BHZ = THETA(7)
  IF(IHR.GT.100.AND.IHR.LE.120) BHZ = THETA(8) ; Hazard depends on HR (IHR in $ERROR)
  IF(IHR.GT.120) BHZ = THETA(9)
IF(FLG.EQ.3)    IPRED = SUR*BHZ   ; Survival * hazard (S*h), conversion
...
$THETA  (0,0.0400)       ; 7 BHAZ FOR CONVERSION, HR <= 100
$THETA  (0,0.0400)       ; 8 BHAZ FOR CONVERSION, 100>HR<=120
$THETA  (0,0.0400)       ; 9 BHAZ FOR CONVERSION, HR > 120
