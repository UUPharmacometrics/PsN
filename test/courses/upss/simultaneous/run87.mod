...
$DES
 ...
 DADT(4) = BHAZ*EXP(-THETA(8)*T)     ; Integrate hazard, hazard depends on TIME
...
IF(FLG.EQ.3)    IPRED = SUR*BHAZ*EXP(-THETA(8)*TIME) ; Survival * hazard (S*h), conversion
...
$THETA  .001             ; 8 Effect of time on hazard
