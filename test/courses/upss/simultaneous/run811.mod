...
$DES
 ...
 DEL= 1E-6
 DADT(4) = LAM*SHP*(T+DEL)**(SHP-1)     ; Integrate hazard
...
$ERROR
 ...
 DELX = 1E-6
 HAZNOW = LAM*SHP*(TIME+DELX)**(SHP-1)

 IF(FLG.EQ.3) THEN
   IPRED = SUR*HAZNOW  ; Survival * hazard (S*h), conversion
   DV1 = 1
 ENDIF
 IF(FLG.EQ.4) THEN
...
$THETA  (0,0.0400)       ; 7 LAMBDA FOR CONVERSION
$THETA  (0,.001)         ; 8 SHAPE
...
$EST MAX=9990 LAPLACIAN MET=1 PRINT=1 MSFO=msfb811   
$COV
;Sim_end

$TAB ID TIME IPRED IWRES IPRED FLG CHZ SUR EVID DV1 ONEHEADER 
     NOPRINT FILE=sdtab811
