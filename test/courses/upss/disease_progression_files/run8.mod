...
;---------------- DRUG CONC & EFFECT -------------------------
  ...
  PROT = 0
  IF(DOSE.GT.0.AND.VISI.GT.3) PROT = THETA(6)
;---------------- MODEL & PREDICTION NA ----------------------
 DS    = BASE*(1+(ALPHA-PROT)*TIME/24/7)*(1-EFF) ; Disease status
...
$THETA  (0,0.001,0.02)  ; 6 PROTECTIVE
...
$COV PRINT=E
$TAB ID TVIS IPRED IWRES              ONEHEADER NOPRINT FILE=sdtab8
$TAB ID BASE IOV                      ONEHEADER NOPRINT FILE=patab8
$TAB ID DOSE ICL                      ONEHEADER NOPRINT FILE=cotab8
$TAB ID TIME TAD DS                   ONEHEADER NOPRINT FILE=mytab8
