...
;---------------- UNDERLYING DISEASE -------------
 ...
 MDS   = THETA(3)                 ; Maximum increase in disease Status
 TPROG= THETA(4)                 ; T1/2 describing disease progression rate
...
;---------------- MODEL & PREDICTION NA ----------------------
 DS    = BASE*(1+MDS*(1+EXP(-LOG(2)/TPROG*TIME/24/7))) ; Disease status
...
$COV PRINT=E
$TAB ID TVIS IPRED IWRES              ONEHEADER NOPRINT FILE=sdtab3
$TAB ID BASE IOV                      ONEHEADER NOPRINT FILE=patab3
$TAB ID DOSE ICL                      ONEHEADER NOPRINT FILE=cotab3
$TAB ID TIME TAD DS                   ONEHEADER NOPRINT FILE=mytab3
