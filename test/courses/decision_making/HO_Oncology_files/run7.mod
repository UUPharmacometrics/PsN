$PROBLEM Weibull - covariates minus ECOG
...
$PK
 ...
 COV   = BETA1*BASET + BETA2*TSR6 + BETA3*META
...
;--------------------------TTE Model ------------------------------
  ...
  COVX   = BETA1*BASET + BETA2*TSR6 + BETA3*META
...
$ESTIM MAXEVAL=9999 METHOD=0 LIKE PRINT=1 MSFO=msfb7 SIGL=9 NSIG=3
$COV PRINT=E
;Sim_end
