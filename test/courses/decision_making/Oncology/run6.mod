$PROBLEM Weibull - covariates minus META
...
$PK
 ...
 BETA2 = THETA(4)                ; coefficient TSR6
 BETA3 = THETA(5)                ; coefficient ECOG

 COV   = BETA1*BASET + BETA2*TSR6 + BETA3*ECOG
...
;--------------------------TTE Model ------------------------------
  ...
  COVX   = BETA1*BASET + BETA2*TSR6 + BETA3*ECOG
...
$THETA    0.8      ; 4. coefficient TSR6
$THETA    0.3      ; 5. coefficient ECOG
$OMEGA  0  FIX

;Sim_start : add for simulation
;$SIMULATION (123456789) (12345 UNIFORM) ONLYSIM NOPRED
$ESTIM MAXEVAL=9999 METHOD=0 LIKE PRINT=1 MSFO=msfb6 SIGL=9 NSIG=3
$COV PRINT=E
;Sim_end
