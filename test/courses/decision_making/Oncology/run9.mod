$PROBLEM Weibull - covariates minus BASET
...
$PK
 ...
 BETA1 = THETA(3)                ; coefficient TSR6
 BETA2 = THETA(4)                ; coefficient ECOG
 BETA3 = THETA(5)                ; coefficient META

 COV   = BETA1*TSR6 + BETA2*ECOG + BETA3*META
...
;--------------------------TTE Model ------------------------------
  ...
  COVX   = BETA1*TSR6 + BETA2*ECOG + BETA3*META
...
$ESTIM MAXEVAL=9999 METHOD=0 LIKE PRINT=1 MSFO=msfb9 SIGL=9 NSIG=3
$COV PRINT=E
;Sim_end
