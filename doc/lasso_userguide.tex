...
\guidetitle{LASSO user guide}{2017-11-16}
...
Covariate models for population pharmacokinetics and pharmacodynamics are often built with a stepwise covariate modelling procedure (scm tool, available in PsN). When analysing a small dataset this method may produce a covariate model that suffers from selection bias and poor predictive performance. The lasso tool \cite{Ribbing} is a method suggested to remedy these problems. It may also be faster than the scm tool and provide a validation of the covariate model. 
