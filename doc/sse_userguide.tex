...
\section{Warnings}
The sse can fill up the disk unless the user is careful. 
The sse will not delete existing \$TABLE from any of the input models. 
This is intentional, because in some applications table output is needed.
The number of table files generated will be the number if \$TABLE multiplied with the number of samples.
Also, if the user has supplied \$SIMULATION in the simulation model, PsN will only change SEED in there, nothing else. 
In particular, NSUBS will not be changed, although NSUBS should not be set at all for the sse. If the user 
accidentally sets NSUBS=N, all output will be multiplied by N.
\end{document}
