...
\usepackage{amsmath}
\guidetitle{FREM userguide}{2016-06-15}
...
Default not set.
If set then script will estimate
the mean values of the covariates, instead of using fixed values equal to the mean values in the data.
This option is intended for the case when there are missing covariate observations. If there are no missing
covariate observations for categorical covariates then there will be numerical difficulties.
\nextopt
\optname{rescale}
Default not used. Rescale covariate ETAs in Model 2 to SD close to 1.
...
If estimation of Model 4 failed to even produce an ofv:
\begin{enumerate}
...
\begin{enumerate}
	\item Copy first observation line for individual. i.e. first line where MDV/EVID ==0. If no such line then just skip this individual.
...
Insert
a full BLOCK(N\_covariates) where initials is, if option -rescale is \emph{not} set,
variance-covariance matrix of covariates computed during creation of dataset, or, if
option -rescale \emph{is} set, the correlation matrix of covariates computed during
creation of dataset.
...
or, if option -estimate\_means was set\\
\$THETA init\_j ; TVcov\_j\\
where init\_j is mean of covariate j computed during creation of new data set, cov\_j is the name of covariate
...
add lines (if option -rescale is \emph{not} set)
...
If option -rescale is set then 
\begin{verbatim}
ETA(orig_etas+j)
\end{verbatim}
is replaced with
\begin{verbatim}
ETA(orig_etas+j)*(sd of baseline covariate)
\end{verbatim}
...
If the last estimation method is classical, set option NONINFETA=1 in the last estimation record.
...
\begin{enumerate}
\item Unless sir was already run (See above in section Model 4 sir):
Run sir
...
%Use model\_7.mod if it exists in the final\_models subdirectory of the frem run directory,
%otherwise use model\_4.mod from the same directory.
\begin{description}
\item[If Model 4 covstep was not successful:] Set sir option\\
-covmat\_input=proposal\_density.cov, where
the file proposal\_density.cov 
is generated by frem and saved in the frem run directory.
\item[If Model 4 covstep was successful:] Consider running fewer than the default five sir iterations, to speed up the
process.
\end{description}
Example if inside the main frem run directory (and if Model 4 had ofv but no covstep):
\begin{verbatim}
sir final_models/model_4.mod -covmat_input=proposal_density.cov
\end{verbatim}

\item Run the post-processing script with options
-sir\_directory and -frem\_directory set to the frem and sir run directories.
Example (assuming outside the frem run directory and sir was run inside the frem directory):
\begin{verbatim}
postfrem -frem_dir=frem_base_lin4 -sir_dir=frem_base_lin4/sir_dir1
\end{verbatim}
Output is generated in a folder named postfrem1 (or other number).
The script currently assumes that sir was run with input model final\_models/model\_4.mod inside the
frem run directory. If sir was run with model\_7.mod or some other model then the script will not work.
Also, in the script it is currently assumed that SEX equal to 1 is male and equal to 2 is female.
...
\item To get the output in a different format (e.g. pdf) edit the plotfrem.R script and (re-)run
\begin{verbatim}
