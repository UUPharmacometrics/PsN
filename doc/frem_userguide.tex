...
\usepackage{amsmath}
\guidetitle{FREM userguide}{2016-05-19}
...
In frem $\bar{\eta}_{cov}$ are the observed values of the covariates.

The distribution of $\bar{\eta}_{par}$ conditional on $\bar{\eta}_{cov}= \bar{a}_{cov}$,
where $\bar{a}_{cov}$ is known,
is
multivariate normal with mean
\[
\tilde{\mu}_{par}=\bar{\mu}_{par}+\Sigma_{par,cov}\Sigma_{cov,cov}^{-1}\left(\bar{a}_{cov}-\bar{\mu}_{cov}\right)
\]
