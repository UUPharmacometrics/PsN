...
\guidetitle{FREM userguide}{2015-12-04}
...
Example call
\begin{verbatim}
frem run1.mod -time_varying=WT -occasion=VISI -parameters_bov=PHI,LAG 
-invariant=SEX,DGRP
\end{verbatim}
...
A comma-separated list of covariates that are time invariant. Names used in \$INPUT.
...
The non-included ETAs, if any, must have numbers 1 to 'skip\_etas' in the model.
\nextopt
\optname{vpc}
...
\begin{itemize}
    ...
    have ETA number > skip\_etas. In this subset of input model ETAs, the BOV ETAs, if any, must come after all
    BSV ETAs.
    ...
    $<$par$>$= expression using TV$<$par$>$ and possibly BSV/BOV ETA
    \item Parameters listed with option -parameters\_bov must be defined in the model
    following pattern:
    \begin{itemize}
...
\section{Workflow}
\begin{enumerate}
\item Input checks, for example that DV and occasion, if any, are found in \$INPUT.
\item Check if any BOV ETAs already included in model.
...
Skip ETAs with number <= skip\_etas.
For all remaining BOV ETAs, if any,
...
\item Find the set of BSV ETAs as either empty if option -invariant is not set, or 
option -skip\_etas (default 0) up to either the last ETA in model, if there were no BOV ETAs or the
last ETA before first BOV ETA. Check that the first BSV ETA, if any, is the first in a new
\$OMEGA record, otherwise give error message.
...
\item Create frem dataset according to section FREM dataset below.
...
\item Run Model 1.
\item Create Model 2, model with covariates, with parameter BOV if any (pre-existing or added), but without
estimation of covariances between parameter and covariate ETAs, according to section below. FREM dataset.
\item Run Model 2.
\item If estimation of Model 2 failed, create intermediate model 2 BOV, estimate, use initials for Model 2 and
try again.
\item Create FREM model, with covariates, with estimation of all covariances between parameter and covariate ETAs,
according to section below. FREM dataset.
\item Run FREM model, with \$COV step.
...
This models have same number of ETAs as Model 2, but the BSV and BOV blocks are filled in, 1+N\_occasions huge \$OMEGA blocks and reordered ETAs. Take care of initials, use phi-file from Model 2 estimation to get initial values
for covariances in full blocks.
...
\begin{enumerate}
	\item Take input model (FREM Model). Update inits from estimation, if available. Afterwards FIX \$THETA and \$OMEGA. Unfix all \$SIGMA.
...
First take all parameters listed with option -parameters\_bov, since they have had BOV ETAs added. Then find additional parameters, 
