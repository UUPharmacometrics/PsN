...
\guidetitle{FREM userguide}{2015-12-14}
...
Optional, default empty. A comma-separated list of categorical covariates that are to be transformed from
one covariate with Y categories into Y-1 bivariate covariates.
...
between parameter and covariate ETAs.
...
If the input model has BOV, the BOV ETAs must be skipped using this option. 
\nextopt
\optname{estimate\_final\_model}
Default set. If unset with -no-estimate\_final\_model then script will not estimate
the final frem model.
\nextopt
\optname{vpc}
...
%	\item Time-varying covariates must only vary between occasions, not within occasions. 
    ...
    \item All input model ETAs that are not to have covariance with covariate ETAs estimated must have ETA number
    <= skip\_etas. This includes any BOV ETAs in input model.
    \item All input model ETAs that are to have covariance with covariate ETAs estimated must
    have ETA number > skip\_etas. 
%    \item The input model code must not include covariates that are listed in 
%    options -invariant or -time\_varying.
    \item All model parameters (for example CL, V, KA) with ETA number > skip\_etas must be defined using \\
...
\item Check that all BOV ETAs, if any, have number <=  skip\_etas.
An ETA is assumed to belong to BOV occasion 1 if the corresponding \$OMEGA is not SAME, but the next \$OMEGA is SAME. 
An ETA is assumed to belong to BOV occasion 1+$i$ if the corresponding \$OMEGA the $i$th consecutive SAME \$OMEGA.
...
\item Unless input model is already run \emph{and} its phi-file is found,
run a copy of the input model to get estimates and phi-file.
\item Count the number of parameter \$OMEGA blocks, N\_parameter\_blocks, that are needed in the frem model:
Initiate N\_parameter\_blocks = 0. Skip all \$OMEGA for ETA <= skip\_etas.
Then add 1 for each full block \$OMEGA and add N for each diagonal \$OMEGA of size N.
\item Create frem dataset according to section FREM dataset below, include covariate observations 'N\_parameter\_blocks' times.
...
estimation of covariances between parameter and covariate ETAs, according to section below. FREM dataset.
THETA and OMEGA and SIGMA for PK parameters are FIXED in this model. Include \$COV.
\item Run Model 2.
\item Create FREM regular parameterization model,
...
Use Model 2 ETA output in phi-file to compute initial estimates for correlations between parameter and covariate ETAs.
\item Evaluate FREM model with MAXEVAL=0 to get more accurate phi-file ETAs. Recompute correlations
between parameter and covariate ETAS based on phi-file.
\item Cholesky reparameterize \$OMEGA block for parameter-covariate ETAs. FIX
everything in model except parameter-covariate correlation THETAs.
\item Estimate Cholesky frem model, include \$COV step.
\item Take final estimates from Cholesky frem model and transform back to regular \$OMEGA block for FREM regular model.
\item Reestimate FREM regular model (everything or just \$OMEGA??). Run \$COV step.
...
\begin{enumerate} 
    ...
    For each covariate  determine a unique (set of N\_parameter\_blocks) FREMTYPE value(s) > 0.
    ...
    from \$TABLE where the column headers are the same as in \$INPUT but a FREMTYPE column is added with all zeros as values.
    Also add MDV if not EVID or MDV in data set. In the dummy models no columns are DROPped or SKIPped.
...
Loop over each covariate: Add N\_parameter\_blocks new line(s) just before first observation line for individual. 
...
\end{enumerate}
    ...
    covariates. Also compute variance-covariance matrix %of invariant covariates, and of medians (over occasions) for time-varying. Two separate variance-covariance matrices.
    If missing values then just skip.
\end{enumerate}

This dataset is used for Model 2 and frem models.
...
Count number of n\_eps already present (dimension of \$SIGMA in Model 0), set epsnum=n\_eps+1.
Build Model 2 based on copy of Model 0, after updating initial estimates with final estimates
from estimation of Model 0.
...
FIX all \$THETAs from Model 0.
Loop over all covariates, j=1...N\_covariates, with inner loop over k=1...N\_parameter\_blocks, add lines\\
...
j and k is the order number of the parameter block. If median is exactly zero set
...
Loop over covariates j=1...N\_covariates, parameter blocks k=1...N\_parameter\_blocks, add lines
...
%FIXME: repeat N\_parameter\_blocks times???
...
\begin{enumerate}
	\item Base this model on Model 0. %1.
