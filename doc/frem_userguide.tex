...
\usepackage{amsmath}
\guidetitle{FREM userguide}{2020-06-17}
\usepackage{hyperref}
\begin{document}
...
The post-processing does the following:
\begin{enumerate}
...
Use parameter vector sampling given the covariance matrix uncertainty to compute the 5\% and 95\% quantiles of the respective distributions.
...
The above computations are repeated for each sampled parameter vector, to obtain the 90\% confidence interval around the observed
ratios.

Currently exponentiated parameter etas (log-normally distributed parameters) are assumed.
...
\noindent If no uncertainty information could be read the bipp method will be used as a fall back.

If covariance matrix could be read but is not positive-definite, frem can't proceed. This can happen when the cov
