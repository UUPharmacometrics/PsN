...
\usepackage{amsmath}
\guidetitle{FREM userguide}{2016-12-09}
...
\noindent If no uncertainty information could be read postfrem try to perform the above for only one sample, the mean
value (typical individual), so as to fall back on merely population predictions. If matrices are not positive-definite
(which may be the case when cov step did not succeed in NONMEM), they are forced such through setting small eigenvalues
above tolerance value. This is necessary for finding inverse through current Cholesky decomposition implementation. The
impact of this on the results should be small.
