...
\usepackage{amsmath}
\guidetitle{FREM userguide}{2016-07-06}
...
\nextopt
%\optname{mu}
%Default not used. Use mu-referencing for covariate THETAs and ETAs.
%This option is not compatible with reordering of skipped omegas,
%so if -mu is set then the user must ensure that omegas are already ordered
...
Loop over covariates j=1...N\_covariates, %parameter blocks k=1...N\_parameter\_blocks,
add lines\\
Case 1: option -mu is not set and option -rescale is not set:\\
\begin{verbatim}
...
\noindent Case 2: option -mu is not set and option -rescale is set:\\
\begin{verbatim}
IF (FREMTYPE .EQ. j*100) THEN
...
END IF
\end{verbatim}
where $rescaling$ is 1 if option -rescale is not set, otherwise it is the sd of baseline covariate.
...
\noindent To get the postfrem output in a different format (e.g. pdf) edit the plotfrem.R script and (re-)run
\begin{verbatim}
