...
\guidetitle{FREM userguide}{2016-01-14}
...
be the the first 1 to 'skip\_etas' ETAs. 
\nextopt
...
run a copy of the input model (Model 1) to get estimates and phi-file.
...
\\
\emph{Alternative:} include covariate observations 1 time
...
\item Create Model 3, FREM regular parameterization model,
...
\item Evaluate Model 3 with MAXEVAL=0 to get more accurate phi-file ETAs.
\item Recompute correlations
between parameter and covariate ETAS based on phi-file from Model 3 evaluation,
use these to create Model 4, which is same as Model 3 except
with updated initial estimates for OMEGA off-diagonals. Do not estimate Model 4(?).
\item Create Model 5 by
Cholesky reparameterizing the N\_parameter\_blocks \$OMEGA block(s)
for parameter-covariate ETAs in Model 4. FIX
everything in model except parameter-covariate correlation THETAs.
\item Estimate Model 5, include \$COV step.
\item Create Model 6 (final Cholesky FREM model)
...
were FIX in input model (Model 1).
\item Create Model 7 by taking final estimates from Model 5 and 
transforming back to regular \$OMEGA block(s) and FIX everything except
the N\_parameter\_blocks \$OMEGA block(s) for parameter-covariate ETAs.
\item Create Model 8 by copying Model 7 and un-FIXing everything except
parameters that were FIX in input model (Model 1).
...
Estimate Model 7 and Model 8 (FREM regular model) in parallel
(also in parallel with cholesky final model, if requested).
Include \$COV step.
...
\\
\emph{Alternative:} Do not change diagonal omega
...
\\
\emph{Alternative:} Insert only one full BLOCK after last OMEGA for non-skipped ETAs. Do not renumber any BSV eta.
...
only rectangular block from phi, triangles from inits.
\\
\emph{Alternative:} Combine all non-skipped omega to single large block. For covariances between parameter omegas that did
not have covariance in Model 1, set very very small init $10^-6$.
Initial values for covariances between parameter-covariates are computed from
phi-file from estimation of Model 2.

\section{Model 4}
Update parameter-covariate covariances from phi-file from evaluation of Model 3.
\\
\emph{Alternative:}
Keep inits for covariances between parameter-parameter omegas that did
not have covariance in Model 1 very very small.
Update covariances between parameter-covariates from phi-file from evaluation of Model 3. Keep remaining parameter-parameter
covariances to final estimates from Model 1 (fixed in Model 2), and 
covariate-covariate covariances to final estimates from Model 2.

Note: Model 4 is not estimated or evaluated.

\section{Model 5}
Create Model 5 by
Cholesky reparameterizing all non-skipped OMEGA of Model 4.
FIX
everything in model except parameter-covariate correlation THETAs.
\\
\emph{Alternative:} Same except set parameter-parameter correlations to 0 FIX if 0 in Model 1.
