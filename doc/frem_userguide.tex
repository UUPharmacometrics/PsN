...
This option is not allowed in combination with skip\_omegas.
\nextopt
\optdefault{skip\_omegas}{comma-separated list of record order numbers} 
{\Large \texttt{??? TODO: alternative to skip\_etas. If set, then put the skipped omegas as the first ones, renumber all
etas accordingly, and automatically set skip\_etas to correct number. Added as first step setup Model 2}}
\nextopt
...
where there is a single full \$OMEGA blocks with all covariances between parameter and covariate ETAs.
...
Leave all parameter-parameter and covariate-covariate covariances as is.
Set estimation record back to what it was in Model 1.
Unfix the parameter-covariate \$OMEGA block, and unfix everything that was not fix in Model 1.
...
\subsection{TODO}
Automatically add BOV \$OMEGAs to skip\_omegas. Reorder \$OMEGA based on -skip\_omegas, and renumber ETAs. Auto-set skip\_etas. 
Error message if no ETAs remain.
