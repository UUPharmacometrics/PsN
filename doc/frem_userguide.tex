...
\usepackage{amsmath}
\guidetitle{FREM userguide}{2016-05-24}
...
process.
\end{description}
Example (if Model 4 had ofv but no covstep):
\begin{verbatim}
sir final_models/model_4.mod -covmat_input=proposal_density.cov
\end{verbatim}
\item Run the post-processing script with option -rawres\_input set to the final sir raw\_results file. That file is
found inside the sir run directory and has a name matching the sir input model, for example
raw\_results\_model\_4.csv if the input model was named model\_4.mod. Also set option -in\_filter=resamples.gt.0
so that only resampled vectors are used.
...
are assumed.
\end{enumerate}
\item Run the R scripts for frem results visualization.
\end{enumerate}
