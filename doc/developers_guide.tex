...
\guidetitle{Developer's guide}{2015-09-01}
\usepackage{hyperref}
...
\begin{itemize}
	...
	\item If the tests are to be run on Mac, a setup of a temporary directory is needed in includes.pm. 
    Edit test/includes.pm and change the row that sets the \$tempdir to \$tempdir = <my path to an already created temporary directory>;
...
packages texlive-extra-util, texlive-science and texlive-pictures are needed to build all documents.
