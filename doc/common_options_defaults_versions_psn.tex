...
Only relevant if option -nmfe or -nmqual is set (note that -nmfe is sometimes set automatically, see option -nmfe) and NONMEM7.2 or later is used. The string will be copied verbatim into the nmfe script command line. PsN will not check that the options are appropriate. Remember to enclose the string in quotes if it contains any spaces. Note that before PsN 4.1.4 this option was given as a comma separated list of options. What would for PsN 4.1.3 be specified as \verb|-nmfe_option=xmloff,prdefault| must now be specified as \verb|-nmfe_option='-xmloff -prdefault'|. If any psn.conf file with the old format is used when installing PsN it will be automatically updated to the new format.
\nextopt
\optname{nmqual}
