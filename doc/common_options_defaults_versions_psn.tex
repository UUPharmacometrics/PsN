...
Default used. Make PsN run NMtran on the model file before submitting the complete nmfe run to a cluster/grid or forking on a local computer. On a cluster this saves time in the case of syntax errors in the model file, since the user does not have to wait for a slot on the cluster/grid before the error is detected. On a local computer the error handling is improved. PsN will only run NMtran on the first sample in e.g. a bootstrap, sse or randtest.
\nextopt
...
\nextopt
\optdefault{slurm\_project}{string}
