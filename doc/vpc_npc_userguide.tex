...
\guidetitle{VPC and NPC user guide}{2014-12-04}
...
\begin{verbatim}
vpc  run32.mod -samples=500 -bin_by_count=1 -no_of_bins=10
\end{verbatim}
...
is optional, only allowed when -orig\_table is also used. Give file with simulated data as input directly instead of letting the program generate them.  The file must have a header, the first column must be ID and the file must be space separated. 
...
\nextopt
\optname{fine\_pi}
is optional, default not set. If not set, the the prediction intervals computed are 0, 40, 80, 90 and 95\%. If the option is set, then the prediction intervals computed are 0, 10, 20, 25, 30, 40, 50, 60, 70, 75, 80, 90 and 95\%.  
\nextopt
...
Automatic creation of result plots using Xpose.
\nextopt
...
\bibitem{BQL} Bergstrand, M. et al., AAPS J, 2009. 11(2): p. 371-380.
