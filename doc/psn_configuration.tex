...
\guidetitle{PsN configuration}{2014-05-20}
\begin{document}
...
If you have a NMQual8 installation of NONMEM and want to always run NONMEM via the autolog script then you need to replace nmfe=1 with nmqual=1 in the \verb|[default_options]| section in psn.conf. 
...
The option names used in psn.conf are the same as would be used on the command line, and documentation can be found in the document common\_options\_defaults\_versions\_psn.pdf. Some options do not take any value on the commandline, for example -run\_on\_sge, but if set in psn.conf those options must also have a value (1 or 0). If such an option is set in psn.conf, for example
