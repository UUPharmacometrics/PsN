...
Note that -nmqual must be set on the command-line for PsN to run NONMEM using the autolog script.
...
If PsN option -nmqual\_xml is set on the command-line or in psn.conf, then PsN will use that as input to autolog.pl. Otherwise PsN will
use the log.xml file in the nmqual subdirectory of the NONMEM installation directory.
...
When running PsN3 with NMQual the user had to modify the xml-file so that file extension .mod was used instead of .ctl in the 'run' section of log.xml, but PsN4 will instead
rename the local model file to use .ctl as extension when option -nmqual is set.
