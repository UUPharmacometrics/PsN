...
\guidetitle{SIR user guide}{2015-04-21}
\usepackage{amsmath}
...
$\lambda$ less than -3 or larger than +3 (?). Use the $\lambda$ that maximizes the correlation coefficient of
a quantile-quantile plot of the transformed parameter.
...
\item[\underline{Step 9}] (Repeat Step 2) Calculate each vectors' probability, 
use new covariance matrix and Box-Cox transformed parameter vectors for computing each vectors relPDF.
\item[\underline{Step 10}] (Repeat Step 3) Evaluate the back-transformed parameter vectors on the original data.
...
\item[\underline{Step 12}] (Repeat Step 5) Resample 'resamples\_2' parameter vectors based on weights from above step, 
with or without replacement depending on option -with\_replacement. 
Store the number of times each vector was resampled in raw\_results\_2.csv.
\item[\underline{Step 13}] Compute final results from Step 12 vectors on original scale.
\end{itemize}
