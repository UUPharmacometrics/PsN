...
\guidetitle{SIR user guide}{2015-10-13}
\usepackage{amsmath}
...
Either a positive vector with length equal to the number of \emph{estimated} (not FIX) \$OMEGA elements, both diagonal
and off-diagonal, or
...
are sampled from the truncated multivariate normal distribution.
%FIXME
%When off-diagonal $i,j$ is estimated, the correlation will be un-inflated if the 
%inflation vector element is equal to the product of 
The weights will also be computed based the inflated covariance matrix. 
Inflation is not used in any but the first iteration.

NB: There is no check that individual inflation for e.g. OMEGA(2,1) is matched to inflation for OMEGA(1,1) and OMEGA(2,2). 
The user must make sure that the ETA1-ETA2 correlation is inflated/the same/deflated appropriately by setting a suitable 
-omega\_inflation vector.
\nextopt
...
The matrix is inflated if either of options -theta\_inflation, -omega\_inflation or -sigma\_inflation 
is set, and the sampling is done on the original scale. 
...
If -rawres\_input is used there is no raw\_results file for the first iteration. 
If unlimited replacement then sample\_order is the sample order of the last resample. 
In the raw results file there will be '-cap\_resampling' (default 1) identical line(s) for each sample vector.
