...
\guidetitle{SIR user guide}{2015-08-28}
\usepackage{amsmath}
...
the Box-Cox transformed resampled vectors from phase 3 of the previous iteration,
...
as if read from a real raw\_results file. If option -auto\_rawres is used
in combination with rawres\_input, the 'degree' will be used 
...
Step 6 in the previous iteration. Sampling is done on Box-Cox scale, and 
...
output by NONMEM.
NB 2: If $i>0$ or -rawres\_input is used, the vectors (including $\mu$) and covariance matrix are on Box-Cox scale.
...
\item For all but the last iteration: A file boxcox\_covmatrix\_iteration\i.cov with the Box-Cox space covariance matrix for iteration $i$.
...
then it is recommended to either run a new sir with a different random seed, or 
