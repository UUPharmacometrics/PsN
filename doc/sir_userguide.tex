...
\guidetitle{SIR user guide}{2016-08-24}
\usepackage{amsmath}
...
Default not set. Enables experimental fast positive semi-definiteness checks via cholesky
...
%Inflation is always performed so that correlations in the covariance matrix are unchanged.
...
Optional, default is 0 (off). Every N:th (multivariate normal sampling) iteration
will be printed with cumulative acceptance and rejection rates.
\nextopt
\end{optionlist}
...
\end{description}
\subsubsection{RSE - correlation plots for proposal and SIR}
\begin{description}
