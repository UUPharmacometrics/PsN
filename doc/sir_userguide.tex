...
\guidetitle{SIR user guide}{2015-02-19}
...
\item[\underline{Step 1}] Simulate 'samples' parameter vectors from the covariance matrix, i.e. the .cov-file given by NONMEM 
or the matrix given via option covmat\_input. 
...
\begin{math}
\frac{\left(det\left(A^{-1}\right)\right)^{1/2}}{\left(2\pi\right)^{k/2}} exp\left(-\frac{1}{2}(\left(x-\mu\right)A^{-1} \left(x-\mu\right)^T\right)
\end{math}
\\
where $k$ is the number of dimensions, 
$\mu$ is the vector of expectations and $A$ is the (possibly inflated) covariance matrix.
Store parameter values and probabilities in raw\_results.csv.
...
\bf{PDF} & PDF \\
\bf{importance\_ratio} & $\mathrm{likelihood}\mbox{\tt\string_}\mathrm{ratio}/\mathrm{PDF}$\\
...
\subsection*{Failure to invert the covariance file}
If you use the covmat\_input option, and get the error message
\begin{verbatim}
covmat_input file could not be inverted, numerical error 
in linear_algebra invert_symmetric
\end{verbatim}
then it is probable that the the covariance matrix is ill-conditioned and badly scaled. Consider using
the rawres\_input option instead.
\end{document}
