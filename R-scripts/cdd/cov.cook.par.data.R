cov.cook.par.data <- function(cdd.data.all) {
   ...
   cook.par.data <- cdd.data.all[ , grepl("cook.par.", names(cdd.data.all)) & !grepl(".cook.par.", names(cdd.data.all))]
   ...
   for (i in 1:length(parameters)) {
     ...
     se.par.data.col <- as.vector(cdd.data.all[ , grepl("se.", names(cdd.data.all)) & grepl(parameters[i], names(cdd.data.all))])
