outlier.table.ebe.npde <- function(noutlier,outlier_id_row,ebenpde_tmp,ebenpde_obs,index_emp_distance,
                          emp_distance_sort,vector_theor_dist,n.subjects,iiv.eta.names) {
  ...
  if(noutlier > 0){
    ncolunmns <- (3+length(iiv.eta.names))
    fortable <-data.frame()
