summary.table <- function(residual.files,residual.names) {
  ...
  mydataframe <- data.frame('NPDE' = residual.names, mean = round(mymean,digits=5),'p-value (H_0: mean==0)'=round(p_mean_not_0,digits=3),variance=round(variance,digits=5),'p-value (H_0: var==1)'=round(p_var_not_1,digits=3),check.names=FALSE)
  return(mydataframe)
}