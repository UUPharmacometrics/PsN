ofv_summary_table <- function(working.directory,model.filename) {
  ...
  ofv_lin_base_mod_before_est <- .get_ext_ofv(paste0(working.directory,"linearize_run/scm_dir1/",paste0(sub('.([^.]*)$','',model.filename),"_linbase.ext")),0)
  ofv_lin_base_mod <- .get_ext_ofv(paste0(working.directory,"linearize_run/scm_dir1/",paste0(sub('.([^.]*)$','',model.filename),"_linbase.ext")))
  linbase_phi_table <- read.table(paste0(working.directory,"linearize_run/scm_dir1/",paste0(sub('.([^.]*)$','',model.filename),"_linbase.phi")),skip=1,header=T,stringsAsFactors = F)
