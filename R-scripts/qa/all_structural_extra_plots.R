all_structural_extra_plots <- function(working.directory,model.filename,resmod_structural_details,CWRES_table,idv_all,idv_name,dvid_name,type) {
  if(length(idv_all)!=0) {
    for(i in 1:length(resmod_structural_details)) {
      ...
      vpc_tables_list <- get_tables_for_vpc(obs_table=CWRES_table,obs_extra_table=file.path(working.directory,sub('.([^.]*)$','',model.filename),"_linbase.dta"),sim_table=file.path(working.directory,"/simeval_run/m1/sim_res_table-1.dta"),sim_extra_table=file.path(working.directory,"/simeval_run/m1/orig_pred.dta"),idv_all,resmod_structural_details[[i]]$dvid,dvid_name)
