resmod_structural_details_tables <- function(working.directory,model.filename,CWRES_table,idv_all,idv_name) {
  ...
  if(length(idv_all)!=0) {
    #check if dvid exist
    resmod_file_exists <- get_resmod_table(directory=working.directory, idv_all[1])$resmod_file_exists
    if(resmod_file_exists) {
      resmod_table <- get_resmod_table(directory=working.directory, idv_all[1])$resmod_table
