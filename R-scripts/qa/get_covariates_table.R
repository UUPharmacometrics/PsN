get_covariates_table <- function(frem_table,scm_table,max_scm_table) {
  ...
  if((scm_table$dOFV[1] == "ERROR" || scm_table$dOFV[1] == "NA") && length(scm_table$dOFV)==1) {
    ...
    scm_table$Coef <- format(scm_table$Coef,trim=T,digits=1,nsmall=2,scientific=F)
    ...
    if(class(frem_table$dOFV)=="character") {
      covariates_extra_table$dOFV <- format(as.numeric(covariates_extra_table$dOFV),trim=T,digits=1,nsmall=1,scientific=F)
