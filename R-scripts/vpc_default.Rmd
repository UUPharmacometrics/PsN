...
if (!exists('mix')) {     # A mixture model is a special case
    ...
    if (require("vpc")) {
        ...
        plots_plain <- vpc_mixtures(obs=obs, sim=sim, numsims=samples, mixcol=mix, dv=dv)
        plots_phm <- vpc_mixtures(obs=obs, sim=sim, numsims=samples, mixcol=mix, dv=dv, phm_obs=phm_obs_file, phm_sim=phm_sim_file)
