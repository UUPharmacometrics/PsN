    ...
    if (require("vpc")) {
        ...
        for (i in unique(obs_phm$SUBPOP)) { # Allowing cases with more than 2 mixtures
            ...
            plots[[i]] <- ggplot( ) +  
                          ...
                          geom_text(data=temp_obs, aes(rank, PMIX, label=SUBJECT_NO), size=10) +
                          geom_hline(yintercept=0.5, linetype="dashed", size=2) +
                          theme_classic() +
                          coord_flip() + # required because geom_ribbon cannot use xmin/xmax
						  ...
						  ylab(paste0("IPmix (SUBPOP=", i, ")"))		# ylab since flipped coords
        }
        
        left_axis = grid::textGrob("Sorted ID", gp=grid::gpar(font=2,fontsize=30), rot=90)
        title = grid::textGrob("ID sorted by IPmix and 5-95th percentile per subpopulation", gp=grid::gpar(font=2,fontsize=30))
