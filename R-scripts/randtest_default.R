...
if (have.base.model) {
    ...
    if (PsNR::rplots_level(meta) > 1) {
        ...
        KStestOFV <- function(objdf=NULL,dfRange=seq(0.0001,5,0.01),flNm=NULL,ignore.usm=F,alpha=0.95) {
            ...
            vdf      <- tidyr::gather(vdf, quant, val)

            cmdf  <- tidyr::gather(cmdf, variable, CDF, "abs(deltaOFV)", CHISQ)
            ...
            modvdf       <- tidyr::gather(modvdf, quant, val)

            modcmdf  <- tidyr::gather(modcmdf, variable, CDF, "abs(mod.deltaOFV)", CHISQ)
            ...
            for (i in 1:length(dofv)) {
                imdf <- rbind(imdf, data.frame(bin=mean(bin[i:(i+1)]),deltaOFV=dofv[i]))
            }
            set.seed(657652); dchidf <- dchisq(imdf$bin,df)
            pmdf  <- cbind(imdf, CHISQ=dchidf)
            colnames(pmdf)[2] <- "abs(deltaOFV)"
            pmdf  <- tidyr::gather(pmdf, variable, PDF, "abs(deltaOFV)", CHISQ)
            ...
            pmdf  <- tidyr::gather(pmdf, variable, PDF, "abs(mod.deltaOFV)", CHISQ)
        ...
        vdf <- tidyr::gather(tmp, variable, value, deltaOFV, mod.deltaOFV)

        # Melted data frame for plotting purpose
        mdf <- subset(tidyr::gather(na.omit(df[-c(1:2),]), variable, value, deltaOFV, mod.deltaOFV), abs(value)<10^5)
