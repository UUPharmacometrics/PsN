...
# 1.Structural models
structural_overview <- get_structural_overview_table(directory=working.directory,idv=idv_all,dvid_name,groups,skip)
if(all(skip!="resmod")) {
...
eta_table_boxcox <- get_eta_values(working.directory=file.path(working.directory,"modelfit_run"),param_model="boxcox")
...
eta_table_tdist <- get_eta_values(working.directory=file.path(working.directory,"modelfit_run"),param_model="tdist")
if(exists("tdist_table_orig")) {
...
# 3.Covariate Models
  # 3.1.All (FREM)
frem_table_list <- get_all_covariates(frem_directory=file.path(working.directory,"frem_run"),covariates,categorical,parameters,list_par_var_models$dofv_block,skip)
...
scm_table_list <- get_scm_table(file.path(working.directory,"scm_run/raw_results_scm.csv"),
                                parameters,covariates,categorical,skip)
...
```{asis Add_text_for_structural_section_if_not_skipped, echo=all(skip!="resmod")}
\pagebreak
...
```{asis Add_text_for_covariates_section_if_not_skipped, echo=(all(skip!="frem")||all(skip!="scm"))}
\pagebreak
...
```{asis Add_text_for_residual_error_section_if_not_skipped, echo=all(skip!="resmod")}
\pagebreak
...
```{asis Add_text_for_infl_indiv_section_if_not_skipped, echo=all(skip!="cdd")}
\pagebreak
...
```{asis Add_text_for_outliers_section_if_not_skipped, echo=all(skip!="simeval")}
\pagebreak
