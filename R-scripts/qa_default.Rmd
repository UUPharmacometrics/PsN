...
for(i in 1:length(resmod_suffix)) {
  ...
  print(second_table)
  table <- resmod_structural_details[[i]]$table
  idv_plot <- plot_ipred(table,resmod_suffix)
  print(idv_plot)
  cat("\n\\newpage\n")
}
```
...
if(file.exists(paste0(working.directory,"/postfrem1/frem_ratio.csv")) && 
   file.exists(paste0(working.directory,"/postfrem1/frem_id_ratios.csv")) && 
   file.exists(paste0(working.directory,"/postfrem1/covdata.csv")) &&
   file.exists(paste0(working.directory,"/postfrem1/pardata.csv"))) {
  inTable_frem <- read.csv(file.path(working.directory,"postfrem1","frem_ratio.csv"),header = T, as.is = T)
  frem_id <- read.csv(file.path(working.directory,"postfrem1","frem_id_ratios.csv"), header = T, as.is = T)
  covdata <- read.csv(file.path(working.directory,"postfrem1","covdata.csv"),header = T, as.is = T)
  pardata <- read.csv(file.path(working.directory,"postfrem1","pardata.csv"),header = T, as.is = T)
  if(ID_ratio(frem_id,covdata,pardata)$files_exist) {
  ...
  if(parameter_ratio(inTable_frem,covdata,pardata)$files_exist) {
