...
if(length(idv_all)!=0) {
  ...
  for(i in 1:length(idv_all)) {
    ...
    #captions
    all_captions <- captions_structural(idv_all,idv_name,perc)
    resmod_dofv_table_captions <- all_captions$resmod_dofv_table_captions
    structural_bias_tables_captions <- all_captions$structural_bias_tables_captions
    structural_bias_plots_captions <- all_captions$structural_bias_plots_captions
    vpc_captions <- all_captions$vpc_captions
    ...
    print(first_table)
    cat(resmod_dofv_table_captions[i])
    ...
    print(second_table)
    cat(structural_bias_tables_captions[i])
    ...
    if(ncol(shift_tab)!=1) {
      ...
      print(idv_plot)
      cat("\n\n")
      cat(structural_bias_plots_captions[i])
      if(make_vpc) {
        cat("\n\n")
        #vpc plots
...
if(sd_parcov_file_exists) {
  ...
  print(sd_parcov_tab)
  cat("Unexplained variability in eta of parameters conditional on none, one or all covariates. That is the expected variability with a structural covariate-parameter relationship.")
}
```
