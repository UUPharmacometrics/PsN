...
qa_results <- PsNR::retrieve_qa_results(working.directory, model.filename, psn_options)
```
...
ofv_table <- ofv_summary_table(directory=file.path(working.directory,"linearize_run/scm_dir1/"),model.filename)
```
...
#)
#popover_dt$`Hover over these items` <- kableExtra::cell_spec(
#  paste("Message on", popover_dt$position), # Cell texts
#  popover = kableExtra::spec_popover(
#    content = c("Some content potentially long displayed above", "below", "right", "left"),
#    title = "Even a title here!",                           # title will add a Title Panel on top
#    position = popover_dt$position,
#    trigger = c("click", "hover", "focus", "manual")
#  ))
#kableExtra::kable(popover_dt, escape = FALSE) %>%
...
if(list_par_var_models$boxcox_mod && !boxcox_lambdas_list$param_extra_table_error) {
  cat("\\pagebreak\n\n")
}
cat("## Additional etas\n\n")
if(list_par_var_models$add_etas_mod) {
...
p <- PsNR::prepare_va_cov_plot(qa_results) %>% 
  PsNR::plot_result()
...
```{r plot_iofv_vs_iofv,eval=(PsNR::rplots_level(meta) > 1),results='asis',echo=F,warning=F,message=F,fig.width=11,fig.height=6,fig.keep="high",fig.align="center"}
...
```{r command_line_capture, echo=F, comment=NA}
