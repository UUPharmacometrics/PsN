...
check_package_versions(pkg=c("tidyverse","vpc","xpose","ztable","expm","PEIP","yaml","devtools"),
                       versions=c("1.1.1","0.9.1","0.3","0.1.5","0.999-1","2.0-1","2.1.14","1.13.3"),
                       toolname)
#load functions
...
if(exists("boxcox_lambdas_orig") && !boxcox_lambdas_error) {
  eta_table_boxcox <- get_eta_values(param_model=file.path(working.directory,"modelfit_run/boxcox.mod"),theta_values=boxcox_lambdas_orig[,1:2])
...
if(exists("tdist_table_orig") && !tdist_error) {
  eta_table_tdist <- get_eta_values(param_model=file.path(working.directory,"modelfit_run/tdist.mod"),theta_values=tdist_table_orig[,1:2])
