...
check_package_versions(pkg=c("tidyverse","xpose","ztable","expm","PEIP","yaml","devtools"),
                       versions=c("1.1.1","0.4.0","0.1.5","0.999-1","2.0-1","2.1.14","1.13.3"),
                       toolname)
#load functions
...
cat("#Overview\n\n")
overview_table <- keep_symbols(overview_table_list$overview_table,type)
overview_table <- ztable_sub(overview_table,type=type,include.rownames = F,colnames.bold = T,align="lrc")
overview_table <- addrgroup(overview_table,rgroup=overview_table_list$rgroup_names,n.rgroup=overview_table_list$n.rgroup,cspan.rgroup=1)
...
if(list_par_var_models$fullblock_mod) {
  full_omega_block_table <- keep_symbols(full_omega_block_list$full_omega_block_table,type)
  full_omega_block_table <- ztable_sub(full_omega_block_table,type=type,colnames.bold=T,include.colnames=!full_omega_block_list$full_omega_block_error,include.rownames=F,longtable=T,align="lrr")
...
boxcox_lambdas_table <- keep_symbols(boxcox_lambdas_list$param_extra_table,type)
boxcox_lambdas_table <- ztable_sub(boxcox_lambdas_table,type=type,colnames.bold=T,include.colnames=!boxcox_lambdas_list$param_extra_table_error,include.rownames=F,longtable=T,align="lrrr")
...
cat("##Additional etas\n\n")
if(list_par_var_models$add_etas_mod) {
  add_etas_table <- keep_symbols(add_etas_list$add_etas_table,type)
  add_etas_table <- ztable_sub(add_etas_table,type=type,colnames.bold=T,include.colnames=!add_etas_list$add_etas_error,include.rownames=F,longtable=T,align="lrrr")
...
cat("##t-distribution\n\n")
tdist_table <- keep_symbols(tdist_list$param_extra_table,type)
tdist_table <- ztable_sub(tdist_table,type=type,colnames.bold=T,include.colnames=!tdist_list$param_extra_table_error,include.rownames=F,longtable=T,align="lrrr")
print(tdist_table)
...
tdist_shape_plot <- plot_transformed_density(data_table=list_tdist_shape$eta_transf_table,eta_table_tdist,param_model="tdist")
...
if(list_par_var_models$iov_mod) {
  iov_table <- keep_symbols(iov_list$iov_table,type)
  iov_table <- ztable_sub(iov_table,type=type,colnames.bold=T,include.rownames=F,include.colnames=!iov_list$iov_error,longtable=T,align="llrrr")
...
cat("\\pagebreak\n\n")
cat("#Covariates\n\n")
covariates_extra_table <- keep_symbols(covariates_table_list$covariates_extra_table,type)
...
for (i in 1:length(resmod_table_list$dvid_nr)) {
  ...
  resmod_ruv_models_table <- keep_symbols(resmod_table_list$resmod_ruv_table_list[[i]],type)
  ...
  if(!nonlinear && exists("xpdb")) {
    ...
    res_shrinkage <- keep_symbols(res_shrinkage,type)
    align <- paste(c("l",rep("r",ncol(res_shrinkage)-1)),collapse = '')
    res_shrinkage <- ztable_sub(res_shrinkage,type=type,colnames.bold=T,include.colnames=T,include.rownames=F,longtable=T,align=align)
    print(res_shrinkage)
...
value <- T
if(ncol(ii_list$ii_table)==1 && nrow(ii_list$ii_table)==1) value <- F
ii_table <- keep_symbols(ii_list$ii_table,type)
...
cat("#Outliers\n\n")
value <- T
if(ncol(outlier_table_list$outliers_table)==1 && nrow(outlier_table_list$outliers_table)==1) value <- F
outliers_table <- keep_symbols(outlier_table_list$outliers_table,type)
...
if(all_outl_list$files_exist) {
  simeval_overall_outliers_table <- keep_symbols(all_outl_list$all_outlier_table,type)
  simeval_overall_outliers_table <- ztable_sub(simeval_overall_outliers_table,type=type,colnames.bold = all_outl_list$add_column_names,include.colnames = all_outl_list$add_column_names,include.rownames = F,longtable = T,align="lccccc")
if(all_outl_list$add_column_names) {
  simeval_overall_outliers_table <- addcgroup(simeval_overall_outliers_table,cgroup=all_outl_list$cgroup,n.cgroup=all_outl_list$n.cgroup)
}
  cat("\\pagebreak\n\n")
