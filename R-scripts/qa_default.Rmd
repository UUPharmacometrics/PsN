...
header-includes: \usepackage{colortbl} # delete if html file needed and set type="html"
...
real.mod.prefix <- sub('.mod.*','',model.filename)
```
...
scm_table_list <- get_scm_table(file.path(working.directory,paste0("scm_run/raw_results_",real.mod.prefix,".csv")))
...
get_param_var_table <- function(directory,n.eta) {
  ...
  if(file.exists(paste0(directory,"linearize_run/",real.mod.prefix,"_linbase.ext")) &&
     file.exists(paste0(directory,"modelfit_run/raw_results.csv"))) {
    
    linbase_ofv <- .get_ext_ofv(paste0(directory,"linearize_run/",real.mod.prefix,"_linbase.ext"))
...
list_par_var_models <- get_param_var_table(directory=working.directory,n.eta)
...
#Overview
```{r overview_table,echo=F,results='asis'}
rgroup <- c("Parameter Variability Model","Covariates","Residual Error Model","Influential Individuals","Outliers")
overview_list <- list(par_var_models,covariates_table,resmod_ruv_overview,cdd_highest_dofv,max_outlier_table)
n.rgroup <- c()
...
full_omega_block_table <- update_ztable(full_omega_block_table,type="latex",colnames.bold = T,include.rownames = F)
...
boxcox_lambdas_table <- update_ztable(boxcox_lambdas_table,type=type,colnames.bold = T,include.rownames = F)
...
covariates_extra_table <- update_ztable(covariates_extra_table,type=type,colnames.bold = T,include.rownames = F)
print(covariates_extra_table)
```

#Residual Error Model
```{r resiual_error_model,echo=F,results='asis'}
nr_rows <- grep("_",resmod_ruv_models_table[,1])
for(i in 1:length(nr_rows)) {
  resmod_ruv_models_table[nr_rows[i],1] <- gsub("_"," ",resmod_ruv_models_table[nr_rows[i],1])
}
resmod_ruv_models_table <- ztable(resmod_ruv_models_table)
resmod_ruv_models_table <- update_ztable(resmod_ruv_models_table,type=type,colnames.bold = T,include.rownames = F)
...
ii_table <- update_ztable(ii_table,type=type,colnames.bold = T,include.rownames = F)
print(ii_table)
```
...
outliers_table <- update_ztable(outliers_table,type=type,colnames.bold = T,include.rownames = F)
