...
for (i in 1:length(resmod_table_list$dvid_nr)) {
  ...
  if(!nonlinear && exists("xpdb")) {
    res_var_attribution <- try(resmod_variability_attribution(xpdb,dvid_col_name=dvid_name,dvid_value=resmod_table_list$dvid_nr[i],smooth=TRUE))
    if(!inherits(res_var_attribution, "try-error")){
