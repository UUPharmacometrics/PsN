...
if(add_etas_mod) {
  add_etas_table <- get_add_etas_table(directory=working.directory,added_etas=added_etas,dofv_add.etas=list_par_var_models$dofv_add_etas)
}
if(iov_mod) {
  iov_table <- get_iov_table(directory=working.directory,iov_etas=iov_etas,dofv_iov=list_par_var_models$dofv_iov)
}
if(fullblock_mod || boxcox_mod || tdist_mod || add.etas_mod ||iov_mod) {
  param_sector <- TRUE
} else {
  param_sector <- FALSE
}
...
# 3.Covariate Models
  # 3.1.All (FREM)
frem_table_list <- get_all_covariates(frem_directory=file.path(working.directory,"frem_run"),contionuous,categorical,parameters,list_par_var_models$dofv_block,skip)
...
scm_table_list <- get_scm_table(scm_directory=file.path(working.directory,"scm_run"),
                                ...
                                parameters,contionuous,categorical,skip)
...
if(add_etas_mod) {
  ...
  add_etas_table <- ztable_sub(add_etas_table,type=type,colnames.bold=T,include.rownames=F,longtable=T,align="lrrr")
...
if(iov_mod) {
  ...
  iov_table <- ztable_sub(iov_table,type=type,colnames.bold=T,include.rownames=F,longtable=T,align="llrrr")
