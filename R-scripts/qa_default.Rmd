...
for (i in 1:length(resmod_ruv_models_table_list)) {
  if(length(dvid_nr) ==1 && dvid_nr == 'NA') {
    ...
    resmod_ruv_models_table <- keep_symbols(resmod_ruv_models_table_list[[i]],type)
    resmod_ruv_models_table <- ztable_sub(resmod_ruv_models_table,type=type,colnames.bold = T,include.rownames = F,longtable = T,align="lrc",caption = paste0(dvid_name,"=",dvid_nr[i]))
