...
#error table
error_table <- function(first_column="",col=2) {
  nr_rows <- length(first_column)
  if(col > 2) {
    error.table <- data.frame(matrix(c("ERROR",rep("",col-2)),1,(col-1)))
    error.table <- cbind(first_column,error.table)
    colnames(error.table) <- rep("",col)
  } else if(col == 2){
    error.table <- data.frame(first_column,"ERROR")
    colnames(error.table) <- rep("",col)
  } else {
    error.table <- data.frame("ERROR")
    colnames(error.table) <- ""
  }
  return(error.table)
}
...
resmod_structural_models <- as.list(resmod_suffix) %>%
