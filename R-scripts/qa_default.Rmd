...
if(file.exists(CWRES_table) && 
   file.exists(file.path(working.directory,sub('.mod.*','',model.filename),"_linbase.dta")) &&
   file.exists(file.path(working.directory,"simeval_run/m1/sim_res_table-1.dta"))) {
  make_vpc <- T
...
for(i in 1:length(resmod_suffix)) {
  ...
  idv_plot <- plot_ipred(shift_tab,resmod_suffix[i])
  print(idv_plot)
  if(make_vpc) {
    cat("\n\n")
    #vpc plots
    vpc_plot <- plot_structural_vpc(obs,sim,shift_tab,idv=resmod_suffix[i])
    print(vpc_plot)
  }
  cat("\n\\newpage\n")
}
