...
# obs <- vpc::read_table_nm(CWRES_table)
# extra_obs <- vpc::read_table_nm(paste0(working.directory,"/pheno_real_linbase.dta"))
# obs <- left_join(obs,extra_obs[,c("ID","DV","TIME")])
# add_cols <- obs[,c("ID",resmod_suffix)]
# sim <- vpc::read_table_nm(paste0(working.directory,"/simeval_run/m1/sim_res_table-1.dta")) %>% left_join(add_cols)
...
for(i in 1:length(resmod_suffix)) {
  ...
  print(idv_plot)
  #vpc plots
  # vpc_plot <- plot_structural_vpc(table1,table2,shift_tab,idv=resmod_suffix[i])
  # print(vpc_plot)
  cat("\n\\newpage\n")
}
```
