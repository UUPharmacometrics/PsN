...
xpdb <- xpose4::xpose.data(xpose.runno, directory=results.directory, tab.suffix=tab.suffix, mod.prefix=model_prefix, mod.suffix=model_suffix)
...
if (PsNR::rplots_level(meta) > 1){
  ...
  if (res.table != '') {
    file_1_exists <- file_existence_in_directory(directory=results.directory, file_name=paste0(model_prefix, xpose.runno, ".phi"))
    file_2_exists <- file_existence_in_directory(directory=results.directory, file_name=res.table)
    
    if ((file_1_exists) && (file_2_exists)) {
      # calculate data
      list_out <- data.obj.obsi(obj.data.dir=paste0(results.directory, model_prefix, xpose.runno, ".phi"),
