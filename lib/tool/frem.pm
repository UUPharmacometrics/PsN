...
{ 
  ...
  if (not $self->model3()){
      ...
      $self->bsv_parameters($frem_model0->problems->[0]->nomegas( with_correlations => 0,
								  with_same => 1));
...
#		   %subargs );
	  ...
	  my $eta_matrix = $phi -> get_eta_matrix( n_eta => $self->bsv_parameters()) if (defined $phi);
...
#	  print "using ETA cov omega fill\n";
	  $BSV_par_block = $frem_model1-> problems -> [0]->get_full_omega(covmatrix => $eta_covariance_0);
#	  print "done omega fill\n";
      }else{
#	  print "using empirical omega fill\n";
	  $BSV_par_block = $frem_model1-> problems -> [0]->get_full_omega();
      }
      $frem_model1-> problems -> [0]->omegas([]); #verify!
...
# line 1306 "lib/tool/frem_subs.pm" 
...
sub create_data2 {
	...
	# Start of Non-Dia code #
        'debug' -> warn(level => 3, message => "Entering \t" . ref($self). '-> create_data2');
# line 1079 "lib/tool/frem_subs.pm" 
{
    #in ref of model, 
...
# line 1622 libgen/tool/frem.pm 
        'debug' -> warn(level => 3, message => "Leaving \t" . ref($self). '-> create_data2');
	# End of Non-Dia code #
...
sub set_frem_records {
	...
	# Start of Non-Dia code #
        'debug' -> warn(level => 3, message => "Entering \t" . ref($self). '-> set_frem_records');
# line 799 "lib/tool/frem_subs.pm" 
{
    #in is ref of model
...
# line 1946 libgen/tool/frem.pm 
        'debug' -> warn(level => 3, message => "Leaving \t" . ref($self). '-> set_frem_records');
	# End of Non-Dia code #
...
# line 1368 "lib/tool/frem_subs.pm" 
{
...
# line 1334 "lib/tool/frem_subs.pm" 
{
  #input is integers $column_index, $start_row_index, $end_row_index 
...
# line 1289 "lib/tool/frem_subs.pm" 
{
