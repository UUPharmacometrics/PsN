...
# 2004-08-26
# Verkar som om kovariatnamnen måste vara unika. (Se substr i
# _option_val_pos i problem_subs) Annars kan kovariater som används
# droppas när en annan kovariat med kortare namn som börjar lika dant
...
start gof_crc
	...
	if ($self->linearize() and $self->update_derivatives()){
	  $base_n_param = 0;
	}
	...
	foreach my $crit ( 'iiv_CL', 'iiv_V' ) {
	  foreach my $model ( @{$self -> prepared_models->[$model_number-1]{'own'}} ) {
	    ...
	    my $ref = $model -> get_values_to_labels($category => omega);
	    $crc_changes{$crit} = $ref -> [0][0][$omega_number{$crit}];
	  }
	}
...
end create_user_code

# }}} create_user_code

# {{{ format_inits_bounds

start format_inits_bounds
      {
	my $median = $statistics{'median'};
	my $mean = $statistics{'mean'};
	my $max = $statistics{'max'};
...
end add_code_linearize

# }}} add_code_linearize

# {{{ add_code_gfunc

start add_code_gfunc
      {
