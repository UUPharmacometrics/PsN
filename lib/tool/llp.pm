...
sub modelfit_analyze
{
    ...
    if ( $active ) {
        if ( $self -> max_iterations > 1 ) {
            ...
            my $internal_llp =
                tool::llp ->new( %{common_options::restore_options(@common_options::tool_options)},
                                 normq              => $self->normq,
        ...
        foreach my $num ( @par_nums ) {
            ...
            foreach my $side ( 'lower', 'upper' ) {
                ...
                if ( not $done ) {
                    ...
                    if ( $active_flag ) {
                        my $new_mod = model -> new( %{common_options::restore_options(@common_options::model_options)},
                                                    directory   => $model_dir,
                                                    filename    => $filename,
