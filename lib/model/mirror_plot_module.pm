...
sub BUILD
{
    ...
    my $msfo_names = $base_model -> msfo_names( ); #array over prob
    my $msfi_names = $base_model -> msfi_names(absolute_path =>1 );
    ...
    if( $base_model -> is_run and $self->mirror_from_lst ) {
        ...
        $base_model -> remove_option( record_name => 'estimation',
                                      option_name => 'MSFO',
                                      problem_numbers => [1]);
        $base_model -> remove_option( record_name => 'nonparametric',
                                      option_name => 'MSFO',
                                      problem_numbers => [1]);

        $prob_num = 1;

    } elsif( ( defined $msfo_names -> [0] and -e $msfo_names -> [0])
             or
             ( defined $msfi_names -> [0] and -e $msfi_names -> [0]) ){

        my $have_msfi=0;

        if( defined $msfi_names -> [0] and -e $msfi_names -> [0] ){
            $have_msfi = 1;
        }
        ...
        unless( $have_msfi ){
            ...
            $truefinal = ' TRUE=FINAL ';

            my $msfo_name = $msfo_names -> [0];
        ...
        # 3. Add $MSFI
        $truefinal = ' TRUE=FINAL ';
        $base_model -> problems->[1]->msfis([model::problem::msfi->new(record_arr => ['msfo'],
    ...
    $base_model -> set_records( type => 'simulation',
                                ...
                                record_strings => ["($seed) NSUB=$nr_of_mirrors $truefinal"] );
