...
#		  "statements when computing median and other values for\n".
...
#	$this->max_data_items(17); #default is 50
#	print "experiment max data 17\n";
...
sub _raw_results_callback {
	...
	# Start of Non-Dia code #
        'debug' -> warn(level => 3, message => "Entering \t" . ref($self). '-> _raw_results_callback');
# line 1300 "lib/tool/scm_subs.pm" 
      {
...
#	  my $local_line_structure = $modelfit->raw_line_structure; 
...
#	      $j += ($n_diag_cols+$npar_orig{'theta'});
	  ...
	  if ( $step_number == 1 ) {
		...
		1;
#	      }elsif (not ( $max_hash{$param} > 1 or ref $res eq 'ARRAY' )) {
...
# line 2570 libgen/tool/scm.pm 
        'debug' -> warn(level => 3, message => "Leaving \t" . ref($self). '-> _raw_results_callback');
	# End of Non-Dia code #
...
# line 2996 "lib/tool/scm_subs.pm" 
      {
...
#or 		  ($self->data_items() > $self->max_data_items)){ 
...
#		unless ($self->data_items() > $self->max_data_items);
	  }
	  $start_model -> _write();
	  ...
	  if ( defined $start_model->outputs() and 
	    ...
	    my $ofvname = 'ofv';
#	    $ofvname = 'DIC' if 
#		( defined $start_model-> outputs -> [0] -> get_single_value(attribute=> 'dic')); #error
...
#	    and not ($self->data_items() > $self->max_data_items)){
	  #there is no option for setting copy data
	  $subargs{'data_path'}='../../';
	  unless ($self->update_derivatives()){
	    my $maxlev = 9;
...
sub linearize_setup {
	...
	# Start of Non-Dia code #
        'debug' -> warn(level => 3, message => "Entering \t" . ref($self). '-> linearize_setup');
# line 1897 "lib/tool/scm_subs.pm" 
      {
	  my $linearize_only=0;
	  if (($self->max_steps() == 0) and ($self->step_number()==1) and 
	      scalar(keys %{$self->test_relations()}) == 0){
...
#	  $derivatives_model -> filename('derivatives.mod');
	   ...
	   #3.2, 3.3
		  ...
		  if (defined ($eta_parameter{$i})){
			  ...
			  $cov_count++;
#		my $cname='COV'.$cov_count;
#		push(@pred_block,$cname.'=D_ETA'.$i.'*1*(GZ_'.$param.'-OGZ_'.$param.')');
		      }
		  }
	      }
...
# line 4032 libgen/tool/scm.pm 
        'debug' -> warn(level => 3, message => "Leaving \t" . ref($self). '-> linearize_setup');
	# End of Non-Dia code #
...
# line 3394 "lib/tool/scm_subs.pm" 
    {
      return if ($self->return_after_derivatives_done());
...
#	if ( not defined $final_model or $final_model eq 'basic_model' ) { #bug, replace
	...
	if ($self->step_number() == 1){
	  if (defined $self -> initial_estimates_model){
	    ...
	    1;
	    #if we have included_relations this will not be the right model, do nothing
#	    $self->write_final_models(final_model => $self -> models->[$model_number-1],
#				      model_number => $model_number);
	  }
	}
...
sub gof_ofv {
	...
	# Start of Non-Dia code #
        'debug' -> warn(level => 3, message => "Entering \t" . ref($self). '-> gof_ofv');
# line 4318 "lib/tool/scm_subs.pm" 
      {
	...
	foreach my $model ( @models ) {
	  ...
	  if ($model -> outputs -> [0]->have_output()){
	    ...
	    #$ofvname = 'DIC' if
		#( defined $model-> outputs -> [0] -> get_single_value(attribute=> 'dic'));#error
...
# line 4739 libgen/tool/scm.pm 
        'debug' -> warn(level => 3, message => "Leaving \t" . ref($self). '-> gof_ofv');
	# End of Non-Dia code #
...
sub gof_pval {
	...
	# Start of Non-Dia code #
        'debug' -> warn(level => 3, message => "Entering \t" . ref($self). '-> gof_pval');
# line 4468 "lib/tool/scm_subs.pm" 
      {
	my ( $direction, $basic_model, $model_number, $ofv_ch_ref ) = @_;
...
# line 5025 libgen/tool/scm.pm 
        'debug' -> warn(level => 3, message => "Leaving \t" . ref($self). '-> gof_pval');
	# End of Non-Dia code #
...
sub calculate_categorical_statistics {
	...
	# Start of Non-Dia code #
        'debug' -> warn(level => 3, message => "Entering \t" . ref($self). '-> calculate_categorical_statistics');
# line 5062 "lib/tool/scm_subs.pm" 
{
  if (defined $model){
    $data=$model->datas->[0];
  }
...
# line 5120 libgen/tool/scm.pm 
        'debug' -> warn(level => 3, message => "Leaving \t" . ref($self). '-> calculate_categorical_statistics');
	# End of Non-Dia code #
...
sub calculate_continuous_statistics {
	...
	# Start of Non-Dia code #
        'debug' -> warn(level => 3, message => "Entering \t" . ref($self). '-> calculate_continuous_statistics');
# line 5116 "lib/tool/scm_subs.pm" 
      {
	if (defined $model){
	  $data=$model->datas->[0];
	}
...
# line 5213 libgen/tool/scm.pm 
        'debug' -> warn(level => 3, message => "Leaving \t" . ref($self). '-> calculate_continuous_statistics');
	# End of Non-Dia code #
...
# line 4751 "lib/tool/scm_subs.pm" 
      {
...
sub create_code {
	...
	# Start of Non-Dia code #
        'debug' -> warn(level => 3, message => "Entering \t" . ref($self). '-> create_code');
# line 5891 "lib/tool/scm_subs.pm" 
      {
	my $new_code = [];
	my $new_inits = [];
	my $new_bounds = {};
	...
	if ( scalar @code == 1){
#	  print "$parameter $covariate ".$code[0]."\n"; 
	  if( $code[0] eq 'none' ) {
	    $codetype = 1;
...
#	print "type $codetype state $state\n";
...
# line 5756 libgen/tool/scm.pm 
        'debug' -> warn(level => 3, message => "Leaving \t" . ref($self). '-> create_code');
	# End of Non-Dia code #
...
sub add_code {
	...
	# Start of Non-Dia code #
        'debug' -> warn(level => 3, message => "Entering \t" . ref($self). '-> add_code');
# line 6033 "lib/tool/scm_subs.pm" 
      {
#	croak("Undefined definition_code input to add_code") 
#	    unless (defined $definition_code);
...
# line 5970 libgen/tool/scm.pm 
        'debug' -> warn(level => 3, message => "Leaving \t" . ref($self). '-> add_code');
	# End of Non-Dia code #
...
sub add_code_linearize {
	...
	# Start of Non-Dia code #
        'debug' -> warn(level => 3, message => "Entering \t" . ref($self). '-> add_code_linearize');
# line 6205 "lib/tool/scm_subs.pm" 
      {
...
#	print "$parameter $covariate theta init ".$inits[0]."\n";
...
# line 6208 libgen/tool/scm.pm 
        'debug' -> warn(level => 3, message => "Leaving \t" . ref($self). '-> add_code_linearize');
	# End of Non-Dia code #
...
sub add_code_gfunc {
	...
	# Start of Non-Dia code #
        'debug' -> warn(level => 3, message => "Entering \t" . ref($self). '-> add_code_gfunc');
# line 6401 "lib/tool/scm_subs.pm" 
      {
	...
	foreach my $parameter (keys %parameter_G){
	  ...
	  if ($parameter_relation{$parameter} eq 'exponential'){
	    ...
	    push(@code,'OGK_'.$parameter.
		 "=1\n");
#	  }elsif ($parameter_relation{$parameter} eq 'none'){
#	    push(@code,'OGK_'.$parameter.
#		 '=1'."\n");
	  }else{
...
# line 6297 libgen/tool/scm.pm 
        'debug' -> warn(level => 3, message => "Leaving \t" . ref($self). '-> add_code_gfunc');
	# End of Non-Dia code #
...
sub run_xv_pred_step {
	...
	# Start of Non-Dia code #
        'debug' -> warn(level => 3, message => "Entering \t" . ref($self). '-> run_xv_pred_step');
# line 3293 "lib/tool/scm_subs.pm" 
    {
...
#      $model_copy_pred -> directory($base_directory);
...
# line 6435 libgen/tool/scm.pm 
        'debug' -> warn(level => 3, message => "Leaving \t" . ref($self). '-> run_xv_pred_step');
	# End of Non-Dia code #
...
sub format_inits_bounds {
	...
	# Start of Non-Dia code #
        'debug' -> warn(level => 3, message => "Entering \t" . ref($self). '-> format_inits_bounds');
# line 5841 "lib/tool/scm_subs.pm" 
      {
...
#	print "stats max $max min $min median $median\n";
...
# line 6521 libgen/tool/scm.pm 
        'debug' -> warn(level => 3, message => "Leaving \t" . ref($self). '-> format_inits_bounds');
	# End of Non-Dia code #
...
sub create_linear_code {
	...
	# Start of Non-Dia code #
        'debug' -> warn(level => 3, message => "Entering \t" . ref($self). '-> create_linear_code');
# line 5169 "lib/tool/scm_subs.pm" 
      {
#	print "create linear code\n";
	...
	if ( $continuous ) {
	  ...
	  if ( scalar @code < 1 or ( scalar @code == 1 and $code[0] eq '') 
		...
		if ( $first_non_missing ) {
		  if ($self->linearize()){
		    #uncomment for experiment, switch
#		    my $miss = $self -> missing_data_token;
#		    if ($statistics{'have_missing_data'}){
#		      $fraction=($factors{$sorted[$i]}+$factors{$miss})/$sum_values;
#		    }else{
...
#		    push @code, $comment."IF($covariate.EQ.$miss) $parameter$covariate".
#			"_COMMON=1; Missing data, set to most common\n" if ($statistics{'have_missing_data'});
		  }else{
		    push @code, $comment."IF($covariate.EQ.$sorted[$i]) $parameter$covariate".
			" = $offset  ; Most common\n";
		  }
...
#	    print "init[i] defined=".$inits[$i]."\n";
	  }
	}
	$end_theta = --$theta_number;
      }
# line 6842 libgen/tool/scm.pm 
        'debug' -> warn(level => 3, message => "Leaving \t" . ref($self). '-> create_linear_code');
	# End of Non-Dia code #
...
sub create_hockey_stick_code {
	...
	# Start of Non-Dia code #
        'debug' -> warn(level => 3, message => "Entering \t" . ref($self). '-> create_hockey_stick_code');
# line 5651 "lib/tool/scm_subs.pm" 
      {
...
# line 7008 libgen/tool/scm.pm 
        'debug' -> warn(level => 3, message => "Leaving \t" . ref($self). '-> create_hockey_stick_code');
	# End of Non-Dia code #
...
sub write_log {
	...
	# Start of Non-Dia code #
        'debug' -> warn(level => 3, message => "Entering \t" . ref($self). '-> write_log');
# line 6456 "lib/tool/scm_subs.pm" 
      {
	my %parm = @_;
...
# line 7128 libgen/tool/scm.pm 
        'debug' -> warn(level => 3, message => "Leaving \t" . ref($self). '-> write_log');
	# End of Non-Dia code #
...
sub drop_undrop_covariates {
	...
	# Start of Non-Dia code #
        'debug' -> warn(level => 3, message => "Entering \t" . ref($self). '-> drop_undrop_covariates');
# line 6571 "lib/tool/scm_subs.pm" 
      {
...
# line 7198 libgen/tool/scm.pm 
        'debug' -> warn(level => 3, message => "Leaving \t" . ref($self). '-> drop_undrop_covariates');
	# End of Non-Dia code #
...
sub write_final_models {
	...
	# Start of Non-Dia code #
        'debug' -> warn(level => 3, message => "Entering \t" . ref($self). '-> write_final_models');
# line 3914 "lib/tool/scm_subs.pm" 
      {
...
#	print "fname ".$$final_model -> filename()." ".$final_model -> directory( $fdir)."\n";
...
#	print "updating from own output object\n";
	my $prob_num = undef;
#	$prob_num = 1 if ($self->data_items() > $self->max_data_items);
	...
	if ($self->linearize()){
	  ...
	  if ($self->update_derivatives()){
		...
		if (-e $outf2){
		  ...
		  #print "error: could not find $outf or $outf2\n";
		}
	      }
#	      print "updating from linearized\n";
...
# line 7365 libgen/tool/scm.pm 
        'debug' -> warn(level => 3, message => "Leaving \t" . ref($self). '-> write_final_models');
	# End of Non-Dia code #
...
# line 4048 "lib/tool/scm_subs.pm" 
      {
...
sub read_config_file {
	...
	# Start of Non-Dia code #
        'debug' -> warn(level => 3, message => "Entering \t" . ref($self). '-> read_config_file');
# line 1234 "lib/tool/scm_subs.pm" 
    {
...
# line 7574 libgen/tool/scm.pm 
        'debug' -> warn(level => 3, message => "Leaving \t" . ref($self). '-> read_config_file');
	# End of Non-Dia code #
...
sub create_exponential_code {
	...
	# Start of Non-Dia code #
        'debug' -> warn(level => 3, message => "Entering \t" . ref($self). '-> create_exponential_code');
# line 5444 "lib/tool/scm_subs.pm" 
      {
	my $theta_number = $start_theta;
	my $median = $statistics{'median'};
	$median = sprintf "%6.2f", $median;
...
# line 7690 libgen/tool/scm.pm 
        'debug' -> warn(level => 3, message => "Leaving \t" . ref($self). '-> create_exponential_code');
	# End of Non-Dia code #
...
sub create_power_code {
	...
	# Start of Non-Dia code #
        'debug' -> warn(level => 3, message => "Entering \t" . ref($self). '-> create_power_code');
# line 5515 "lib/tool/scm_subs.pm" 
      {
	my $theta_number = $start_theta;
	if ($statistics{'min'} < 0){
	  ui -> print( category => 'scm',
...
# line 7816 libgen/tool/scm.pm 
        'debug' -> warn(level => 3, message => "Leaving \t" . ref($self). '-> create_power_code');
	# End of Non-Dia code #
...
sub create_user_code {
	...
	# Start of Non-Dia code #
        'debug' -> warn(level => 3, message => "Entering \t" . ref($self). '-> create_user_code');
# line 5771 "lib/tool/scm_subs.pm" 
      {
	my $median = $statistics{'median'};
	$median = sprintf "%6.2f", $median;
	$median =~ s/\s*//;
...
# line 7932 libgen/tool/scm.pm 
        'debug' -> warn(level => 3, message => "Leaving \t" . ref($self). '-> create_user_code');
	# End of Non-Dia code #
...
sub create_state1_code {
	...
	# Start of Non-Dia code #
        'debug' -> warn(level => 3, message => "Entering \t" . ref($self). '-> create_state1_code');
# line 5598 "lib/tool/scm_subs.pm" 
      {
	my $theta_number = $start_theta;
	my $median = $statistics{'median'};
	$median = sprintf "%6.2f", $median;
...
# line 8030 libgen/tool/scm.pm 
        'debug' -> warn(level => 3, message => "Leaving \t" . ref($self). '-> create_state1_code');
	# End of Non-Dia code #
...
sub preprocess_data {
	...
	# Start of Non-Dia code #
        'debug' -> warn(level => 3, message => "Entering \t" . ref($self). '-> preprocess_data');
# line 821 "lib/tool/scm_subs.pm" 
    {
...
# line 8517 libgen/tool/scm.pm 
        'debug' -> warn(level => 3, message => "Leaving \t" . ref($self). '-> preprocess_data');
	# End of Non-Dia code #
